...
static void initUdp() {
    ...
    clieAddrUDP.sin_addr.s_addr = inet_addr("192.168.137.1");
...
static void sendUdp(const char* data, int length) {
    int rtnValue = sendto(clifdUDP,data,length,0,(struct sockaddr *)&clieAddrUDP,len);
}

void MPU6050_daemon_task(void *pvParameters)
{
    ...
    /*while(1) {
        gpio_set_level(ANBT_I2C_SCL, 0);
        printf("high\n");
        delay_ms(1000);
        gpio_set_level(ANBT_I2C_SCL, 1);
        printf("low\n");
        delay_ms(1000);
    }*/
...
//        printf("%ld\n",timestamp);
        if((sensors & INV_WXYZ_QUAT))
        {
			...
			//printf("%3f,%3f,%3f\n",Pitch,Roll, Yaw);
            sendUdp(buffer, sprintf(buffer, "%3f,%3f,%3f\n",Pitch,Roll, Yaw));
        }
...
//							printf("%d,%d\r\n",accel_res[1][1],accel_res[0][1]);
            ...
            //Send_Data(vel_show,disp_show);  // 这句话会导致崩溃
